...
export const newOrderCod = asyncHandler(async (req, res) => {
  ...
  for (const cartItem of cart) {
    ...
    if (cartItem.quantity > product.stock) {
      throw new ApiError(400, null, `${product.title} is out of stock`);
    }
...
export const getAllOrdersAdmin = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "admin") {
    throw new ApiError(403, null, "Access denied. You are not an ADMIN!");
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, orders, "Newest orders fetched successfully"));
});
...
export const getMyOder = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ApiError(403, null, "You are not authorized to view this order");
  }
...
export const updateStatus = asyncHandler(async (req, res) => {
  ...
  if (!allowedStatuses.includes(status)) {
    throw new ApiError(400, null, "Invalid order status");
  }
  ...
  //  Update delivery date/time every time order is marked as Delivered
  if (status === "Delivered" ) {
  ...
  // Remove delivery date if order is moved back from Delivered
  if (status !== "Delivered") {
...
export const newOrderOnlinePayment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(500, null, "Failed to create Stripe payment session");
  }
});
...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (!sessionId) {
    throw new ApiError(400, null, "Stripe session ID is required");
  }
  ...
  if (session.payment_status !== "paid") {
    throw new ApiError(400, null, "Payment has not been completed");
  }
